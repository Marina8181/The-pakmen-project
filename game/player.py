...
class BasePlayer:
    ...
    def move_by(self, dx, dy):
        # Перемещает каждую ось отдельно (При этом оба раза проверяются коллизии.)
        if dx != 0:
    ...
    def move_single_axis(self, dx, dy):
        ...
        for wall in self.linked_level.get_walls():
            if self.rect.colliderect(wall.rect):
                if dx > 0:  # Вправо; Врезаемся слева
                    self.rect.right = wall.rect.left
                    self.vector[0] = 0
                if dx < 0:  # Влево; Врезаемся справа
                    self.rect.left = wall.rect.right
                    self.vector[0] = 0
                if dy > 0:  # Вниз; Сверху врезаемся
                    self.rect.bottom = wall.rect.top
                    self.vector[1] = 0
                if dy < 0:  # Вверх; Снизу врезаемся
...
class EnemyPatrol(BasePlayer):
    def __init__(self, level, patrol):
        ...
        self.patrol = patrol  # Ключевые места посещения
        self.current_target = 0  # Индекс мест
        self.path = []    # список всех точек между текущей точкой патрулирования и следующей точкой.
        # path работает как стек. индекс 0 всегда является следующей квадратной целью и исчезает при достижении следующей цели.
        self.timer = PatrolTimer(0)
        # Таймер застывания врага
    ...
    def update(self):
        ...
        if self.timer.is_active():
            ...
            # Проверяем, достигнута ли следующая плитка, и если да, то переходим к следующей по пути.
    ...
    def step_path(self): # Метод удаления прошлой точки из списка и обновления пути
        self.path.pop(0)
        if len(self.path) > 0:
            self.update_vector()
    ...
    def update_vector(self):
        if self.timer.is_active():
            ...
            if game.get_tile_position(self.linked_level, self.rect.topleft)[0] < self.path[0][0]:
                ...
                #сравнивает текущую позицию объекта относительно первой точки пути и устанавливает соответствующий вектор
...
def get_random_patrol(level, spawn, points=10): # генерирует случайный патрульный маршрут, который включает в себя перемещения между случайно выбранными точками на уровне, с возможными случайными паузами в патрульном движении.
    random_patrol = [spawn]
    while len(random_patrol) < points:
        ...
        if game.GridPath(level.format, spawn, point, constants.WALL_FORMATS).get_path() is not None and \
                point != random_patrol[len(random_patrol)-1]:
...
class EnemySeeker(EnemyPatrol):
    def __init__(self, level, player, spawn):
        ...
        self.active = False  # Преследование
    ...
    def update_path(self):
        self.path = game.GridPath(self.format,game.get_tile_position(self.linked_level, self.rect.topleft),game.get_tile_position(self.linked_level, self.target_player.rect.center),constants.WALL_FORMATS).get_path()
